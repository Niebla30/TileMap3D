...
func _physics_process(delta: float) -> void:
	...
	if input_vec != Vector2.ZERO:
		...
		# $$V_{iso} = \text{Vector2}(input.x - input.y, (input.x + input.y) \cdot 0.5).normalized()$$
		#var direccion_iso = Vector2(input_vec.x - input_vec.y, (input_vec.x + input_vec.y) * 0.5).normalized()
		...
		var cell: Vector2i = Global.world_to_grid_corregido(global_position, altura)
		cell = Vector2i(clamp(cell.x, 0, Global.MAP_SIZE.x - 1), clamp(cell.y, 0, Global.MAP_SIZE.y - 1))
		if cell != cell_actual:
			print(cell)
			var tile = Global.pool[Global.grid_id(cell)]
			cell_actual = cell
			altura = tile.height
			#$Sprite2D.position.y = -tile.height
			global_position.y = tile.global_position.y
